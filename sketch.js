...
function setup() {
  ...
  fakeClouds = createSprite(width/2,height-40,width*2,20);
...
function draw() {
  ...
  if(gameState === "choose"){
    ...
    text("Which Quest Shall You Take?",width/2-50,height-600);
    ...
    text("Don't Get Caught     Get above 100000",width/4-60,height/2 +height/8);
    text("Get Above 100000    Dont Finish Your Ammo",width/2+width/8-130,height/2+height/8);
  ...
  if(gameState === "water"){
    ...
    if(frameCount % 10 === 0){
     ...
     // poseidon.velocityY = 6;
      ...
      if(poseidon.isTouching(oceanBottom)){

        poseidon.velocityY = 4;

      } else if(poseidon.isTouching(oceanTop)){
    ...
    if( ammo === 0 && score < 3000 || sharkGroup.collide(poseidon)){
  ...
  if(gameState === "sky"){
    ...
    if((touches.length > 0  || keyDown("space")) && zues.y >= height/2 + height/4 + 30){
    ...
    if(enemyGroup.collide(zues) && score<1000){
